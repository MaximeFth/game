...
for (int i=0; i < input.height; ++i){
  for (int j=0; j < input.width; ++j){
      if(brightness(input.pixels[i*input.width+j])==255){
        ...
        //take the min of valid label
          if(label1 == -1){label1 = infinity;}
          if(label2 == -1){label2 = infinity;}
          if(label3 == -1){label3 = infinity;}
          if(label4 == -1){label4 = infinity;}
...
for(int i=0; i<labels.length; ++i){
  ...
  if (!onlyBiggest){
    ...
    for(int i=0; i<output.width*output.height; ++i){
         if(labels[i]==maxIndex){ output.pixels[i]=Color.WHITE.getRGB();}
         else{output.pixels[i]=Color.BLACK.getRGB();}
    }
