...
void draw() {
  ...
  PImage edge=img1.copy();
  PImage blob=img1.copy();

//IMAGE DE GAUCHE ---------------------------------------------------------------

  // Hue/Brightness/Saturation thresholding:
  img2 = thresholdHSB(img2, 80, 135, 40, 255, 20, 255);
...
//IMAGE MILLIEU (EDGE)----------------------------------------------------------
  ...
  //thresholdHSB
  edge = thresholdHSB(edge, 80, 137, 40, 255, 40, 255);

  //blurring:
  edge=gaussBlurr(edge);

  //edge detection:
  edge= scharr(edge);

  //suppression of pixels with low brightness:
  edge= threshold(edge, 140);

  image(edge, img1.width, 0);
...
//IMAGE DROITE (BLOB)-----------------------------------------------------------
  ...
  image(blob, 2*img1.width, 0);
}
//END DRAW-------------------------------------------------------------------

void plotLines(List<PVector> lines, PImage edgeImg) {
  List<PVector> t=new ArrayList();
  QuadGraph Q = new QuadGraph();
  t=Q.findBestQuad(lines, edgeImg.width, edgeImg.height, edgeImg.width*edgeImg.height, edgeImg.width*edgeImg.height/10, false);  //jijijijijjijijjijijijijdiajfiajfiajfiajfidjasfiajifdjsaifjasijfdiasjfidasjfiasjfiajfiajfiajidsjfiajfdiasjfiasjdfiajidfjasifjaisfjaisjfiasjfiasjfsaisf
  for (int j=0; j<t.size(); ++j) {
    fill(color((int)(255*random(0, 1)), (int)(255*random(0, 1)), (int)(255*random(0, 1))), 80);
    ellipse(t.get(j).x, t.get(j).y, 55, 55);
  }
